    ...
    updatePlaySelected(play, game) {
        $.ajax({
            ...
            success: function (response) {
                ...
                window.location.href = '/' + game + '/plays/' + play
            }
        });
    }

    updatePlayerSelected(player, game) {
        $.ajax({
            url: '/game/' + game,
                ...
                <MenuList subheader={<ListSubheader><center><h6>Most Used Play</h6></center></ListSubheader>} className={styles.left}>
                    {most_used.map((play) => (
                        <div>
                            <MenuItem
                                key={play.key}
                                onClick={() => this.updatePlaySelected(play.id, game)}
                            >
                ...
                <MenuList subheader={<ListSubheader><center><h6>Most Efficient Player</h6></center></ListSubheader>} className={styles.right}>
                    {players.map((player) => (
                        <div>
                            <MenuItem
                                key={player.key}
                                onClick={() => this.updatePlayerSelected(player.id, game)}
                            >
